...
**Status:** Complete (2026-06-04T10:00:00Z) — verify-plan.py 16/16 pass; 59/59 unit tests pass; live install on this machine yields a clean global with all 13 agents + command + mcp + provider + permission + enabled_providers, manifest carries `provider_snapshot: null` / `permission_snapshot: null`, and uninstall on the null snapshots correctly DELETES the keys (not restore from post-merge value). Committed in `04c668a`.
